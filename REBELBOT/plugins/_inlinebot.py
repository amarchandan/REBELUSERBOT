    ...
    buttons.append(
        [
            custom.Button.inline(
               f"⬅️ᏴᎪᏟᏦ {REBEL_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "@REBELBOT_SUPPORT":
            ...
            result = builder.article(
                "@REBELBOT_SUPPORT",
                text="""**[{DEFAULTUSER}](tg://user?id={REBEL})**\n\n📜 __No.of Plugins__ :`{len(CMD_HELP)}`\n🗂️ __Commands__ : `{len(apn)}`\n**page:** 1/{veriler[0]}",""",
                buttons=[
                    [
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO SIR. KYA BHNCHODE KAB SE TAP TAP KIYA JA RAHA HA JA KK KHUD KA BANA LO. © REBELBOT ™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"[{DEFAULTUSER}](tg://user?id={REBEL})**\n\n📜 __No.of Plugins__ :`{len(CMD_HELP)}`\n🗂️ __Commands__ : `{len(apn)}`\n**page:** 1/{veriler[0]}",",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            REBEL_alert = "HELLO SIR. KYA BHNCHODE KAB SE TAP TAP KIYA JA RAHA HA JA KK KHUD KA BANA LO. © REBELBOT ™"
    ...
    async def Information(event):
        ...
        buttons.append([custom.Button.inline("⬅️ᏴᎪᏟᏦ", data=f"page({page})")])
        await event.edit(
