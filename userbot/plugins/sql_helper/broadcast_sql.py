...
class REBELBroadcast(BASE):
    __tablename__ = "REBELbroadcast"
    ...
    def __repr__(self):
        return "<REBEL Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, REBELBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


REBELBroadcast.__table__.create(checkfirst=True)

REBELBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with REBELBROADCAST_INSERTION_LOCK:
        broadcast_group = REBELBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with REBELBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(REBELBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with REBELBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(REBELBroadcast.keywoard)
            .filter(REBELBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(REBELBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(REBELBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(REBELBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(REBELBroadcast).all()
        for x in all_groups:
