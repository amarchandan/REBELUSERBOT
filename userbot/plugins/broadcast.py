...
async def REBELbroadcast_send(event):
    if event.fwd_from:
        return
    REBELinput_str = event.pattern_match.group(1)
    if not REBELinput_str:
    ...
    await REBELevent.edit(resultext)
    if BOTLOG:
...
async def REBELbroadcast_add(event):
    if event.fwd_from:
        return
    REBELinput_str = event.pattern_match.group(1)
    if not REBELinput_str:
...
async def REBELbroadcast_remove(event):
    if event.fwd_from:
        return
    REBELinput_str = event.pattern_match.group(1)
    if not REBELinput_str:
...
async def REBELbroadcast_list(event):
    if event.fwd_from:
        return
    REBELinput_str = event.pattern_match.group(1)
    if not REBELinput_str:
    ...
    await edit_or_reply(REBELevent, finaloutput)
...
async def REBELbroadcast_delete(event):
    if event.fwd_from:
        return
    REBELinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(REBELinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {REBELinput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(REBELinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {REBELinput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
